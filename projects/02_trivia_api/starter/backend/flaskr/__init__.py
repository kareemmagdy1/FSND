...
def create_app(test_config=None):
    ...
    def create_question():
        ...
        try:
            ...
            if (len(question.question) == 0 | len(question.answer) == 0):
                abort(400)
            question.insert()
   ...
   '''
    ...
    def get_questions_by_category(categoryId):
        ...
        questions = db.session.query(Question).filter(Question.category == categoryId).all()
        questionsData = []
        for q in questions:
            questionsData.append(q.format())
        totalQuestions = len(questions)
    ...
    def get_next_question():
        ...
        if (category['type'] == 'click'):
            ...
            if (len(previousQuestion) == 0):
                ...
                choosenQuestion = db.session.query(Question).filter(
                    int(category['id']) + 1 == Question.category).first()
                if (choosenQuestion == None):
                    abort(500)
                return jsonify({
                    'question': choosenQuestion.format()
                })
                ...
                index = len(previousQuestion) - 1
                newQuestion = db.session.query(Question).filter(int(category['id']) + 1 == Question.category,
                                                                Question.id > previousQuestion[index]).first()
                if (newQuestion == None):
                    return jsonify({
                        'question': None
                    })
